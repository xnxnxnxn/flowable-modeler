    ...
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', '$window', 'editorManager',
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
                ...
                var currentData = data.stencils[stencilIndex];
                // 只有设置能显示的才可以显示
                if (!currentData.canShow) {
                    continue;
                }
            ...
            editorManager.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    const taskIndex = selectedItem.properties.findIndex(v => v.key === 'oryx-tasklisteners');
                    if (taskIndex !== -1) {
                        const listen = selectedItem.properties[taskIndex];
                        if (!listen.value) {
                            const defaultFields = {
                                name:"roleName",
                                stringValue:"r$管理员"
                            ...
                            const defaultListen = {
                                event: 'create',
                                className: 'cn.net.rococo.hrms.listener.CommonTaskHandler',
                                implementation:'cn.net.rococo.hrms.listener.CommonTaskHandler',
                                fields: [defaultFields]
                            }
                            listen['value'] = {
                                taskListeners: [defaultListen]
                            }
                            $scope.updatePropertyInModel(listen);
                        }
        ...
        $scope.updatePropertyInModel = function (property, shapeId) {
            const nameIndex = $scope.selectedItem.properties.findIndex(v => v.key === 'oryx-name');
            ...
            if (property.key === 'oryx-conditionsequenceflow') {
                // 处理连接线
                const name = $scope.selectedItem.properties.find(v => v.key === 'oryx-name');
                name.value = property.value.expression.showFlowName;
                $scope.updatePropertyInModel(name);


            } else if (property.key === 'oryx-tasklisteners') {
                // 设置监听器
                if ( property.value.taskListeners.length > 0) {
                    const listen = property.value.taskListeners[0];
                    if (listen.fields.length > 0) {
                        const field = listen.fields[0];
                        ...
                        if (value.indexOf('$') !== -1) {
                            ...
                            if (arr[0] === 'u') {
                                // 用戶需要把工号去掉
                                name.value = $scope.getUserName(arr[1]);
                            } else {
                                name.value = arr[1] + "审批";
                            }
                        } else {
                        ...
                        $scope.updatePropertyInModel(name);
                                           ...
                                           * 把用户中的工号曲调
                                           */
                                          $scope.getUserName = function (userNameAndJobCode) {
                                              let value = '';
                                              const users = userNameAndJobCode.split(',');
                                              for (let j = 0; j < users.length; j++) {
                                                  const name = users[j].substring(0, users[j].lastIndexOf('('));
                                                  value += name;
                                                  if (j === users.length - 1) {
                                                      value += ' 审批';
                                                  } else {
