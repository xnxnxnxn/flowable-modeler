...
angular.module('flowableModeler').controller('SaveModelCtrl', [ '$rootScope', '$scope', '$http', '$route', '$location', 'editorManager',
    ...
    $scope.save = function (successCallback) {
        ...
        // 验证结果
        let result = true;
        // 如果存在排他网关，需要验证网关的值是否存在重复
        const exclusiveGatewayList = json.childShapes.filter(child => child.stencil.id === 'ExclusiveGateway');
        if (exclusiveGatewayList) {
            exclusiveGatewayList.some(exclusiveGateway => {
                const minAndMaxList = [];
                // 遍历每一个排他网关,得到出线
                exclusiveGateway.outgoing.forEach(outgoing => {
                    const sequenceFlow = json.childShapes.find(child => child.resourceId === outgoing.resourceId);
                    if (sequenceFlow) {
                        minAndMaxList.push(getParseMinAndMax(sequenceFlow.properties.conditionsequenceflow.expression.staticValue));
                    }
                });
                // 根据 expressionList 里面的属性值进行判断
                if (minAndMaxList.length === 1) {
                    result = false;
                    return true;
                }
                // 遍历 [null,3] [6,null] 只能存在一个
                // 得到开始边界
                const startBoundary = minAndMaxList.filter(m => m[0] === null);
                // 得到结束边界
                const endBoundary = minAndMaxList.filter(m => m[1] === null);
                if (startBoundary.length !== 1 || endBoundary.length !== 1) {
                    result = false;
                    return true;
                }
                // 验证值是连续的
                let start = startBoundary[0][1];
                const checkResult = $scope.checkExpression(minAndMaxList, start, endBoundary[0][0]);
                if (!checkResult) {
                    result = false;
                    return true;
                }
        ...
        $scope.checkExpression = function (list, start, end) {
            const newStart = start + 1;
            if (end === newStart) {
                return true;
            }
            const range = list.find(m => m[0] === newStart);
            if (!range) {
                return false;
            }
            return $scope.checkExpression(list, range[1], end);
        };
