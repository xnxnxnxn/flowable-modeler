...
angular.module('flowableModeler').controller('FlowableTaskListenersPopupCtrl',
    ['$scope', '$http', '$q', '$translate', '$timeout', function ($scope,$http, $q, $translate, $timeout) {

        // $scope.example1model = []; $scope.example1data = [ {id: 'David', label: "David"}, {id: 'Jhon', label: "Jhon"}, {id: 'Danny', label: "Danny"} ];

        $scope.nameModel = [];
        $scope.searchSelectAllSettings = { enableSearch: true, showSelectAll: true, keyboardControls: true };
        $scope.selectTranslate = {
            checkAll:'全选',
            uncheckAll: '全不选',
            searchPlaceholder:'搜索',
            buttonDefaultText:'请选择',
            dynamicButtonTextSuffix:'项被选中'
        };
        ...
        $scope.type = 'r';
        // 获取角色信息
        $http.get(FLOWABLE.APP_URL.getRoleUrl())
            .success(function (data, status, headers, config) {
                $scope.roleNames = $scope.transformToObject(data.data);
                // 获取用户信息
                $http.get(FLOWABLE.APP_URL.getUserUrl())
                    .success(function (data, status, headers, config) {
                        $scope.userNames = $scope.transformToObject(data.data);
                        $scope.typeNames =  $scope.roleNames;
                        if ($scope.taskListeners.length > 0) {
                            if ($scope.taskListeners[0].fields.length > 0) {
                                $scope.selectedField = $scope.taskListeners[0].fields[0];
                                if ($scope.selectedField.stringValue.indexOf('$') !== -1) {
                                    const values = $scope.selectedField.stringValue.split('$');
                                    $scope.type = values[0];
                                    $scope.listenerTypeChanged();
                                    $scope.selectedField.stringValue = values[1];

                                }
                            }
                        }
                        // 进行转换
                        if ($scope.selectedField.stringValue != null) {
                            const nameModels = $scope.selectedField.stringValue.split(',');
                            for (var i = 0; i < nameModels.length; i++) {
                                const model = {
                                    id:nameModels[i],
                                    label: nameModels[i]
                                }
                                $scope.nameModel.push(model);
                            }
                        }
                    })
                    .error(function (data) {
        ...
        $scope.transformToObject = function (data) {
            const list = [];
        ...
        $scope.listenerTypeChanged = function () {
            if ($scope.type === 'r') {
                $scope.typeNames =  $scope.roleNames;
            } else {
                $scope.typeNames =  $scope.userNames;
            }
            $scope.nameModel = [];
        };
        ...
        $scope.getStringFormModel = function () {
            const v  = $scope.nameModel.map(v => v.id).join(',');
            return v;
        }

        // Click handler for save button
        $scope.save = function () {
            if ($scope.nameModel.length === 0) {
                alert('请至少选择一个审批人');
                return;
            }
            if ($scope.taskListeners.length > 0) {
                ...
                // 拼接类型
                $scope.taskListeners[0].fields[0].stringValue = $scope.type + '$' + $scope.getStringFormModel();
